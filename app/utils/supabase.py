...
def get_grants_by_keyword(keyword, limit=100, offset=0):
    ...
    try:
        ...
        result = client.table("grants").select("*").like("search_keyword", f"%{keyword}%").range(from_range, to_range).execute()
...
def update_grant_summary(grant_id, summary_data):
    """
    Update a grant record with summary information generated by GPT.
